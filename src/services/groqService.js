...
export const getSmartPlannerData = async (info, pref) => {
  ...
  try {
    ...
    // Dynamic diet instruction script built for Pakistani context
    const dietInstruction = 
      pref.dietType === "Veg" ? "Strictly generate Vegetarian Pakistani meals (No meat, no chicken, no fish. Focus on high-protein lentils/daal, beans, vegetables, chickpeas/chana, eggs if acceptable, paneer, and dairy)." :
      pref.dietType === "Non-Veg" ? "Focus heavily on Pakistani Non-Vegetarian food (High protein chicken, beef, mutton, fish, and eggs while keeping it healthy)." : 
      "Standard Pakistani mix diet (A balanced combination of healthy meat, chicken, seasonal vegetables, rice, roti, and lentils).";
    ...
    const response = await axios.post(
      "https://api.groq.com/openai/v1/chat/completions", 
      {
        ...
        messages: [
          { 
            role: "system", 
            content: `You are a fitness and nutrition expert specialized in Pakistani diets. 
            Diet preference constraint: ${dietInstruction}
            
            Return the response ONLY as a raw JSON object with this exact structure:
            {
              "meals": { "breakfast": "...", "lunch": "...", "dinner": "...", "preWorkout": "..." },
              "exercises": [ { "name": "Exercise Name", "sets": "3", "reps": "12" } ],
              "calories": 2000
          ...
          { 
            role: "user", 
            content: `Generate a fitness and meal plan for: ${info.userName}. Age: ${info.age}, Weight: ${info.weight}kg, Height: ${info.height}cm. Goal/Target: ${pref.bodyPart}. Level: ${pref.level}. Diet Type Selected: ${pref.dietType}.` 
          }
        ]
      }, 
      {
