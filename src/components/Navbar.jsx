...
const Navbar = ({ user }) => {
  ...
  // Real-time listen to plan progress to handle dynamic hiding
  useEffect(() => {
    if (!user) {
      setDbPlan(null);
      return;
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (data.latest_plan) {
          setDbPlan(data.latest_plan);
        }
      }
    }, (error) => {
      console.error("Error reading live tracker:", error);
    });
  ...
  // Logic condition: Hide planner link ONLY IF a plan exists AND it is NOT fully completed yet
  const shouldHidePlanner = dbPlan && dbPlan.completedDays < dbPlan.totalDays;
