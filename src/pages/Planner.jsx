...
import { db, auth } from "../services/firebase.js"; // Path corrected here!
...
const Planner = () => {
  ...
  const [workoutPref, setWorkoutPref] = useState({
    ...
    level: "beginner",
    daysPerWeek: "3",
    dietType: "Mix", // Default value set to Mix
  });
    ...
    <div className="max-w-4xl mx-auto p-8 bg-white shadow-2xl rounded-[2.5rem] mt-10 mb-10">
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          {/* Diet Preference Selection Section */}
          <label className="font-bold block pt-2">Diet Preference:</label>
          <div className="flex gap-2">
            {[
              { id: "Mix", label: "Mix" },
              ...
              <button
                key={diet.id}
                type="button"
                onClick={() => setWorkoutPref({ ...workoutPref, dietType: diet.id })}
                className={`flex-1 py-2 rounded-xl capitalize ${
                  workoutPref.dietType === diet.id
                    ? "bg-green-500 text-white"
                    : "bg-gray-100"
                }`}
              >
